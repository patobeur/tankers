...
let _scene = {
	...
	set_sun: function (active = false) {
		...
		const SunCubeGeometry = new THREE.BoxGeometry(0.3, 0.3, 0.3);
		...
		this.SUN.move = () => {
			if (this.SUN.userData.initiated) {
				if (this.SUN.position.x >= this.SUN.userData.amplitude.range)
					this.SUN.userData.amplitude.sens = -1;
				if (this.SUN.position.x <= -this.SUN.userData.amplitude.range)
					this.SUN.userData.amplitude.sens = 1;
				this.SUN.position.x =
					this.SUN.position.x +
					this.SUN.userData.amplitude.sens * this.SUN.userData.speed;
			}
		};
